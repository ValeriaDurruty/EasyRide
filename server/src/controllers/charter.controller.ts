...
export const getCharters = (req: Request, res: Response) => {
    ...
    connection.query('SELECT marca.nombre AS marca, modelo.nombre AS modelo, modelo.FK_Marca, charter.* FROM charter INNER JOIN modelo INNER JOIN marca ON marca.PK_Marca = modelo.FK_Marca AND modelo.PK_Modelo = charter.FK_Modelo WHERE charter.PK_Charter = ?;', id, (err, data) => {
        if(err) {
            // Registrar el error en la consola
